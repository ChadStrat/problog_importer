...
class C5_xmlItem extends Model {
	...
	public function set_item_description($item){
		$this->description = strip_tags(str_replace('�','',(string)$item->description));
		if(!$this->description){
			$namespaces = $item->getNamespaces(true);
			$child = $item->children($namespaces['excerpt']);
			$this->description = strip_tags(str_replace('�','',(string)$child->encoded));
		}
	}
